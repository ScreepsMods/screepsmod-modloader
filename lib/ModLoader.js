...
class ModLoader {
  ...
  getMods (cb) {
    try {
      let files = glob.sync(path.join(path.resolve(__dirname,'..'),'/**/package.json'))
      let mods = files
        .map(f => ({
          dir: path.dirname(f),
          package: require(f)
      ...
      this.log('failed to load mods')
      throw e
    }
  }

  hasKeywords(pack,keywords){
    return !!pack.keywords && keywords.reduce((l,w)=>l && !!~pack.keywords.indexOf(w),true)
  }
